...
import { resetPasswordTemplate } from "../tempate/resetPasswordTempate.js";
...
const forgetpassword = async (req, res, next) => {
  try {
    ...
    const checkTokenExistence = await PasswordReset.findOne({
      email: email
    });
    ...
    await sendEmail([email], 'Shri Guru Granth Sahib', resetPasswordTemplate({
      resetToken: token
    }), ['gurwinder.singh@simbaquartz.com']);
    return res.json({
...
const resetPassword = async (req, res, next) => {
  try {
    ...
    const tokenEntry = await PasswordReset.findOne({
      token: resetToken
    });
    if (!tokenEntry) {
